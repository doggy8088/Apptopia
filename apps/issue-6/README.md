...
3. **錨點連結**: `#` 開頭，對應文件內標題
4. **內部路徑**: `/` 開頭（如 `/posts/article`），**目前會跳過檢查**

#### 關於內部路徑連結

以 `/` 開頭的連結（例如 `/posts/article-name`）通常是靜態網站的內部路由路徑。由於不同靜態網站生成器（Next.js、Gatsby、Astro 等）有各自的路由機制（動態路由、重寫規則等），工具**無法可靠地檢查這類連結**。

**目前行為**：內部路徑連結會被標記為 `ok`，不會造成檢查失敗。
